...
The historical verification baseline covered the implementation source and native-test workflow at commit `0cc7bd8`; the branch's next commit, `804fa28`, only recorded the already-resolved direct `windows-core` dependency in `Cargo.lock`. The blocker-resolution changes described below resolve code or configuration causes only: native Tauri capture, Linux live insertion, and the updated Build Test packaging have not yet been run. Signed main/release workflows and the base Tauri signing defaults remain unchanged.
...
| Native compile/test matrix  | Passed in the historical run; updated packaging unrun | In the historical [Build Test run](https://github.com/ZebulonRouseFrantzich/Handy/actions/runs/33356984130), native Rust library tests passed on Windows x64, Windows ARM64, macOS Intel, macOS ARM64, Linux x64 on Ubuntu 22.04 and 24.04, and Linux ARM64. That run then failed closed because the fork lacked signing credentials. Build Test now requests unsigned/ad-hoc packaging, but no run of that updated workflow is claimed here. |
...
| -------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Native Tauri settings webview          | Repository configuration corrected; capture unverified | The Nix package and development shell now set `WEBKIT_DMABUF_RENDERER_FORCE_SHM=1`. On this non-NixOS workstation the GUI still requires an appropriate nixGL wrapper, and no successful native Tauri settings capture has been recorded.                                                                                                                                                                                       |
...
| Trusted signed packages and size delta | Unverified in the fork                                 | Native tests passed in the historical Build Test run before packaging failed closed on absent fork credentials. The updated workflow has not run; its Build Test artifacts are unsigned/ad-hoc and cannot prove trusted signed release packaging or provide a comparable signed size delta. Signed main/release defaults remain unchanged.                                                                                      |
