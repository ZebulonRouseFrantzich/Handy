...
- the control has a supported insertion route and the monitoring tier required by that route; and
...
| Linux   | AT-SPI `EditableText.insert_text` with physical monitoring (`at_spi_editable_text`)      | `verified_control` / `verified`      | `observed_insertions_only`         | Requires the pinned target checks, exact affected-range readback, target events, and a verified legacy AT-SPI device listener.                                                        |
| Linux   | AT-SPI `EditableText.insert_text` with semantic-only monitoring (`at_spi_editable_text`) | `verified_control` / `verified`      | `unavailable`                      | Requires stable target, caret, character-count, readback, and target-event evidence. Compatible typing, helper routes, pointer/keyboard observation, and auto-submit are unavailable. |
| Linux   | Focused keyboard tool (`linux_focused_keyboard`)                                         | `guarded_focused_control` / `posted` | `observed_insertions_only`         | Requires the physical tier plus a pinned target and a positively checked, canonical stdin tool. Text is sent in units of at most 16 Unicode scalars.                                  |

GTK4 controls can legitimately omit the non-canonical AT-SPI `Enabled` state. Linux eligibility therefore accepts the canonical `Sensitive` state without requiring `Enabled`, while continuing to require `Focused`, `Sensitive`, `Editable`, non-`Defunct`, `Accessible`, and `Text`, secure-field rejection, a collapsed selection, and route, readback, and monitoring evidence.

Linux chooses the tier from runtime evidence, not the desktop name or session type. If the exact legacy device-listener registration cannot be confirmed, Handy deregisters it and queries the controller before considering semantic-only operation. Downgrade is allowed only when that listener is proven absent, target event subscriptions remain healthy, the registry owner is unchanged, and the target exposes direct `EditableText`. A control exposing only `Text` falls back before arming; the keyboard helper is never used without physical monitoring.

The semantic-only route is deliberately not coediting-capable. Before and after every direct unit of at most 16 Unicode scalars, Handy drains locally buffered semantic events and revalidates the pinned process/object, focused/sensitive/editable/non-secure metadata, collapsed caret, and expected character count. A same-target insertion with no pending Handy effect is reported as an unattributed insertion and stops the session with `mixed_input_unavailable`; it is not attributed to a user or keyboard. Focus, editable/sensitive state, caret, selection, content, target, or registry changes stop output. Exact affected-range readback remains authoritative for text Handy already inserted.

These checks reduce the race window but do not create a causal D-Bus fence. An identical concurrent external effect may be indistinguishable from the expected Handy effect, and a command with no AT-SPI-visible target effect may be missed. The semantic-only route therefore makes no simultaneous-typing or raw-input-observation claim. It remains target-bound: every write uses the captured AT-SPI object rather than current focus, and no later final paste is permitted after arming.
...
- **Linux:** there is no permission prompt. A working AT-SPI accessibility bus, verifiable object/process ownership, and healthy target events are required. Direct `EditableText` controls may use the semantic-only tier when the legacy device listener is proven absent; controls exposing only `Text` and all focused keyboard-tool routes still require physical monitoring.
...
- **Linux reports AT-SPI, monitor, or typing-tool unavailable:** confirm the accessibility bus is running and the target exposes a focused, sensitive, editable, non-secure AT-SPI object with `Accessible`, `Text`, and—on semantic-only hosts—`EditableText`. A collapsed selection and stable caret/character-count metadata are required. Guarded routes additionally require physical monitoring and `Auto`, `wtype`, or `ydotool`; only the stdin forms listed above qualify.
- **The Nix GUI does not render outside NixOS:** launch it through a nixGL wrapper appropriate for the host graphics stack. The Nix package and development shell set `WEBKIT_DMABUF_RENDERER_FORCE_SHM=1`, but they cannot by themselves expose a non-NixOS host's graphics drivers.
- **Output stops after typing or clicking:** inspect `mixed_input_support` in the armed capability. Physical routes accept only classified insertions at the expected caret. Semantic-only Linux reports `unavailable`; any uncorrelated target insertion stops output as `mixed_input_unavailable`, while commands with no accessibility-visible target effect may be undetectable. Navigation, selection, destructive edits, target changes, and uncertain provider events invalidate every route. Recover the speech transcript from History when available and start a new dictation.
...
Audio and recognition remain local. Focused output necessarily gives Handy local access to the captured accessibility control while the session is armed. It pins target identity and locally reads accessibility metadata and the target values or affected ranges required to validate changes; these reads are not used to merge user text into the speech transcript. Physical-monitor routes inspect input events transiently to distinguish Handy output from user edits. Linux AT-SPI subscriptions are bus-wide by event class and are filtered locally to the pinned target and foreign focus. During semantic-only setup, Handy also transiently subscribes to pointer events and exports/probes a device listener; it arms only after the controller registration is proven absent and the pointer subscription and D-Bus match rule are successfully removed. No raw keyboard or pointer listener remains while a semantic-only session is armed. User or foreign event text is classified structurally and is not retained in the speech ledger, status, metrics, or logs.
...
The last published cross-platform verification baseline is commit `4cd9989`. The Linux package used for that guest pass was produced by the unsigned/ad-hoc workflow linked below. The semantic-only Linux route is newer; its interactive Fedora/KDE and cross-platform validation status is listed separately below. Signed main/release workflows and the base Tauri signing defaults remain unchanged.

### Automated checks

| Check                       | Result | Evidence and scope                                                                                                                                                                                                                                                                                                                                                   |
| --------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Rust tests                  | Passed | The final local full suite passed 388 tests with the pinned Nix toolchain.                                                                                                                                                                                                                                                                                           |
| Focused Linux tests         | Passed | All 20 focused Linux policy, metadata, event-filtering, registry-owner, transient-focus, cancellation, modifier, race, and child-process tests passed locally.                                                                                                                                                                                                       |
| Rust quality                | Passed | `cargo clippy --all-targets -- -D warnings`, `cargo fmt`, and repository formatting checks passed locally with the final pinned Nix toolchain.                                                                                                                                                                                                                       |
...
| Native compile/test matrix  | Passed | The final unsigned/ad-hoc [Build Test run](https://github.com/ZebulonRouseFrantzich/Handy/actions/runs/33582216862) passed native Rust library tests, packaging, package audits, and artifact upload on Windows x64, Windows ARM64, macOS Intel, macOS ARM64, Linux x64 on Ubuntu 22.04 and 24.04, and Linux ARM64. This does not exercise signed release packaging. |
| Nix                         | Passed | Local `nix flake check` and `nix build .#handy` passed for x86_64 Linux. The fork's final [Nix run](https://github.com/ZebulonRouseFrantzich/Handy/actions/runs/33385090161) also passed flake evaluation and the full Handy build with the updated nixpkgs pin.                                                                                                     |
...
| Independent review          | Passed | Fresh code and security reviews found no remaining blocker or high-severity issue after the Linux event-filtering, registry-owner, stop-chord, transient-focus, race, cancellation, target-validation, privacy, packaging-isolation, and reusable-workflow permission fixes were applied.                                                                            |

### Ubuntu GNOME Xorg guest matrix

An Ubuntu 24.04 QEMU/KVM guest running GNOME on Xorg exercised the packaged direct AT-SPI route against GTK3 Gedit. The probes reported only application, role, character counts, lifecycle states, and stable reason codes; transcript and user text were not printed.

| Scenario                         | Result               | Observed behavior                                                                                                                                                                                                                                                                           |
| -------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Eligibility and progressive text | Passed               | The route reached `Armed` and `Streaming`; a streaming update was inserted into the captured Gedit text control before finalization. Eligibility setup completed in 112 ms in the measured clean run, below the 500 ms target.                                                              |
| Compatible coediting             | Passed               | An ordinary insertion at the expected caret increased the field from three to four characters and remained present after finalization.                                                                                                                                                      |
| Destructive Backspace            | Passed               | Backspace reduced the field from three to two characters, produced the destructive-edit terminal path, and no later final paste overwrote the preserved field.                                                                                                                              |
| Target change                    | Passed               | Switching from the captured Gedit window to a second empty Gedit window preserved three characters in the original target and left the second target empty.                                                                                                                                 |
| Escape cancellation              | Passed               | Escape cancellation left the focused field unchanged and no late final paste appeared.                                                                                                                                                                                                      |
| Volatile final conflict          | Passed, fail-closed  | A natural-speech fixture produced a 108-character final transcript after an incompatible live prefix; Handy preserved the already delivered prefix, saved History, and reported `FinalConflict` instead of deleting or replacing field text.                                                |
| Configured stop chord            | Automated regression | GNOME Xorg emitted two transient `focused=false`/`focused=true` AT-SPI cycles around the configured stop chord. The final fix debounces only transient same-target focus loss; persistent loss, foreign focus, and `Defunct` remain terminal, and every write still revalidates live focus. |

These results establish one GTK3/Xorg control and the physical tier, not blanket Linux application compatibility. GNOME Wayland may now qualify only through the weaker semantic-only direct route when runtime AT-SPI evidence satisfies every requirement above. Handy does not claim or impersonate `org.gnome.Orca.KeyboardMonitor`, does not use the compositor's Orca-only keyboard API, and does not treat the trigger-based InputCapture portal as passive monitoring.

### Remaining interactive and packaging gaps

| Surface                                | Status                                                                         | Reason                                                                                                                                                                                                                                                                                              |
| -------------------------------------- | ------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Native Tauri settings webview          | Packaged launch passed; capture unverified                                     | The Nix package and development shell set `WEBKIT_DMABUF_RENDERER_FORCE_SHM=1`. The packaged app ran through the appropriate nixGL wrapper on this non-NixOS workstation, but native Wayland window capture and accessibility inspection timed out.                                                 |
| Linux application matrix               | One GTK3/Xorg physical-tier control verified; semantic-only Wayland unverified | GTK3 Gedit on Ubuntu 24.04 GNOME Xorg passed the matrix above. Fedora GNOME Wayland, KDE Plasma Wayland, GTK4, Electron, browsers, office suites, terminals, custom controls, guarded keyboard tools, and semantic-only privacy/invalidation behavior still require published interactive evidence. |
...
| Event-processing latency               | Unverified                                                                     | Clean-route eligibility was measured at 112 ms. The 80 ms p95 event-processing target was not measured.                                                                                                                                                                                             |
| Trusted signed packages and size delta | Unverified in the fork                                                         | The final Build Test run passed on all seven unsigned/ad-hoc packaging targets and uploaded ordinary workflow artifacts. Those artifacts cannot prove trusted signed release packaging or provide a comparable signed size delta. Signed main/release defaults remain unchanged.                    |
