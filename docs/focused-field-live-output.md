...
The historical verification baseline covered the implementation source and native-test workflow at commit `0cc7bd8`; the branch's next commit, `804fa28`, only recorded the already-resolved direct `windows-core` dependency in `Cargo.lock`. Blocker-resolution commits through `1f0cd8b` were revalidated locally and in the fork workflows linked below. Native Tauri capture and Linux live insertion remain environment-limited. Signed main/release workflows and the base Tauri signing defaults remain unchanged.

### Automated checks

| Check                       | Result | Evidence and scope                                                                                                                                                                                                                                                                                                                                                   |
| --------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Rust tests                  | Passed | The final local full suite passed 357 tests with the pinned Nix toolchain. The fork's earlier [Rust test run](https://github.com/ZebulonRouseFrantzich/Handy/actions/runs/33355883210) also passed.                                                                                                                                                                  |
| Focused Linux tests         | Passed | All 17 focused Linux policy, metadata, helper, cancellation, modifier, race, and child-process tests passed locally.                                                                                                                                                                                                                                                 |
...
| Native compile/test matrix  | Passed | The final unsigned/ad-hoc [Build Test run](https://github.com/ZebulonRouseFrantzich/Handy/actions/runs/33385639482) passed native Rust library tests, packaging, package audits, and artifact upload on Windows x64, Windows ARM64, macOS Intel, macOS ARM64, Linux x64 on Ubuntu 22.04 and 24.04, and Linux ARM64. This does not exercise signed release packaging. |
...
| Independent review          | Passed | Fresh code and security reviews found no remaining blocker or high-severity issue after the race, cancellation, target-validation, privacy, packaging-isolation, and reusable-workflow permission fixes were applied.                                                                                                                                                |
...
| -------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Native Tauri settings webview          | Packaged launch passed; capture unverified          | The Nix package and development shell set `WEBKIT_DMABUF_RENDERER_FORCE_SHM=1`. The packaged app ran through the appropriate nixGL wrapper on this non-NixOS workstation without the prior EGL initialization error, but native Wayland window capture and accessibility inspection timed out, so the rendered settings surface was not observed.                                                                               |
| Linux live route matrix                | GTK state cause resolved; live insertion unverified | GTK4's omission of non-canonical `Enabled` is now accepted when canonical `Sensitive` and every remaining eligibility gate pass. GNOME Wayland remains unsupported because the safe legacy `DeviceEventController` monitor cannot register; Handy intentionally does not claim or impersonate `org.gnome.Orca.KeyboardMonitor` or add a modern `KeyboardMonitor` workaround. Monitor failure remains a strict pre-arm fallback. |
