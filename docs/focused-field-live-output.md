...
The current verification baseline is commit `4cd9989`. The Linux package used for the final guest pass was produced by the unsigned/ad-hoc workflow linked below. Signed main/release workflows and the base Tauri signing defaults remain unchanged.
...
These results establish one GTK3/Xorg control, not blanket Linux application compatibility. GNOME Wayland remains unsupported because the safe legacy `DeviceEventController` monitor cannot register; Handy intentionally does not claim or impersonate `org.gnome.Orca.KeyboardMonitor` or add a modern `KeyboardMonitor` workaround.
...
| Linux application matrix               | One GTK3/Xorg control verified             | GTK3 Gedit on Ubuntu 24.04 GNOME Xorg passed the matrix above. GTK4, Electron, browsers, office suites, terminals, custom controls, guarded keyboard tools, and Wayland remain unverified.                                                                                       |
